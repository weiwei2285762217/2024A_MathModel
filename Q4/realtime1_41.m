...
n = 100;  % 数据长度为100
m = 100;  % 100台风机
load('Tshaft2_v2.mat','Tshaft2')
load('Ft2_v2.mat','Ft2')
temperature_data = Ft2(1901:2000,1:m);
L_NF2 = zeros(n, m);  % 存储L_p值，二维矩阵
D_FF2 = zeros(n, m);  % 存储Df_p值，二维矩阵
...
save('L_NF2.mat','L_NF2')
save('D_FF2.mat','D_FF2')
% % 定义定时器
...
toc
% 定义寻找局部极大值和极小值的函数
function [L_NF2,D_FF2] = findExtrema(current_index, turbine_index,raw_data,L_NF2,D_FF2)
...
%     rainflowCounting(current_index,turbine_index);
    ...
    if Ptrmin(turbine_index) < 1 || Ptrmax(turbine_index) < 1
        L_NF2(current_index, turbine_index) = L_NF2(current_index-1, turbine_index);
        D_FF2(current_index, turbine_index) = D_FF2(current_index-1, turbine_index);
        return;
    end
...
%         disp(['风机 ', num2str(turbine_index), ' 的 L_p = ', num2str(L_p)]);
        L_NF2(current_index, turbine_index) = L_p;
        ...
        D_FF2(current_index, turbine_index) = Df_p;
