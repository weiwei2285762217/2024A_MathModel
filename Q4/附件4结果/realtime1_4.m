...
n = 300;  % 数据长度为100
m = 10;  % 100台风机
load('Tshaft2_v3.mat','Tshaft2')
load('Ft2_v3.mat','Ft2')
temperature_data = Ft2(1:n,1:m);
L_NF1 = zeros(n, m);  % 存储L_p值，二维矩阵
D_FF1 = zeros(n, m);  % 存储Df_p值，二维矩阵
...
save('L_NF1.mat','L_NF1')
save('D_FF1.mat','D_FF1')
% % 定义定时器
...
toc
% 定义寻找局部极大值和极小值的函数
function [L_NF1,D_FF1] = findExtrema(current_index, turbine_index,raw_data,L_NF1,D_FF1)
...
%     rainflowCounting(current_index,turbine_index);
    ...
    if Ptrmin(turbine_index) < 1 || Ptrmax(turbine_index) < 1
        L_NF1(current_index, turbine_index) = L_NF1(current_index-1, turbine_index);
        D_FF1(current_index, turbine_index) = D_FF1(current_index-1, turbine_index);
        return;
    end
...
%         disp(['风机 ', num2str(turbine_index), ' 的 L_p = ', num2str(L_p)]);
        L_NF1(current_index, turbine_index) = L_p;
        ...
        D_FF1(current_index, turbine_index) = Df_p;
