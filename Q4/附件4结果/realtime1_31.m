...
load('Ft2_v4.mat','Ft2')
temperature_data = Tshaft2(1:n,1:m);
L_NT2 = zeros(n, m);  % 存储L_p值，二维矩阵
D_FT2 = zeros(n, m);  % 存储Df_p值，二维矩阵
...
save('L_NT2.mat','L_NT2')
save('D_FT2.mat','D_FT2')
% % 定义定时器
...
toc
% 定义寻找局部极大值和极小值的函数
function [L_NT2,D_FT2] = findExtrema(current_index, turbine_index,raw_data,L_NT2,D_FT2)
...
%     rainflowCounting(current_index,turbine_index);
    ...
    if Ptrmin(turbine_index) < 1 || Ptrmax(turbine_index) < 1
        L_NT2(current_index, turbine_index) = L_NT2(current_index-1, turbine_index);
        D_FT2(current_index, turbine_index) = D_FT2(current_index-1, turbine_index);
        return;
    end
...
%         disp(['风机 ', num2str(turbine_index), ' 的 L_p = ', num2str(L_p)]);
        L_NT2(current_index, turbine_index) = L_p;
        ...
        D_FT2(current_index, turbine_index) = Df_p;
