function [c,prev_stress] = realtime_three_point_rainflow(stress_window,prev_stress)
...
%     stress = extrema_values_sorted;  % 确保为列向量
    ...
    means = [];
    countnum = 0;
    stress = [prev_stress(:); stress_window(:)];
%     if length(stress)<3
%         stress = [0;0;stress];
%     end
    ...
    while length(stress) >= 3
        ...
        for i = 1:length(stress) - 2
            ...
            if deltaS1 <= deltaS2
                ...
                stress_old(extrema_locs_sorted(i:i+1))=[];
...
%                 countnum = countnum+2;
    ...
    prev_stress = stress_old;
    c= [count' ranges' means'];
    % 显示结果
%     disp('循环计数:');
%     disp(count);
%     disp('循环幅值:');
