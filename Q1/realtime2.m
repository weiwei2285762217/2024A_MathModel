...
function findExtrema(current_index, turbine_index)
    ...
    % 进行雨流计数并计算
    rainflowCounting(turbine_index);
end

% 定义雨流计数函数并在每次循环后进行计算
function rainflowCounting(turbine_index)
    global current_index n minima_stack maxima_stack Ptrmax Ptrmin half_cycle_count full_cycle_count cycle_data L_N D_F;
    theta_b = 5e7;
    mm = 10;
    C = 9.77e70;
