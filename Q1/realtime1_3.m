...
figure()
plot(1:100,log10(Tshaft(102,:)))
hold on
plot(1:100,log10(D_F(100,:)))
legend('real','pre')
% ylim([0 2e-17])
%%
figure()
plot(1:100,D_F(:,50))
%%
filename = '附件5-问题一答案表.xlsx';
xlswrite(filename, D_F, '主轴疲劳数据', 'B2');
%%
% 定义全局变量，用于存储数据和计数
global  L_N D_F;
tic
...
n = 100;  % 数据长度为100
m = 100;  % 100台风机
temperature_data = Tshaft(1:n,1:m);
...
toc
% 定义寻找局部极大值和极小值的函数
function [L_N,D_F] = findExtrema(current_index, turbine_index,raw_data,L_N,D_F)
