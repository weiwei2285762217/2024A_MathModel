...
function rainflowCounting(current_index,turbine_index,minima_stack,maxima_stack,Ptrmin,Ptrmax)
    ...
    persistent theta_b mm C N_total last_count;
    global L_N D_F
    % 如果变量还未初始化，则初始化为0
    if isempty(theta_b)
        theta_b = 5e7;
        ...
        last_count = zeros(1, m);  % 每台风机的完整循环计数器
...
%     disp(['风机 ', num2str(turbine_index), ' 的完整循环次数: ', num2str(full_cycle_count(turbine_index))]);
    ...
    if last_count(turbine_index)<count
        ...
        D_F(current_index, turbine_index) = Df_p+D_F(current_index-1, turbine_index);
...
%     if last_count(turbine_index)<count
%         % 计算S
%         if count-last_count(turbine_index)>1
%             Df_p = 0;
%             for tempcount = last_count(turbine_index)+1:1:count
%                 
%                 S = cycle_data(tempcount, 2, turbine_index) ./ (1 - cycle_data(tempcount, 3, turbine_index) ./ theta_b);
%                 N = C ./ S.^m;
%                 % 计算Df_p
%                 Df_p = sum(cycle_data(tempcount, 1, turbine_index) ./ N)+Df_p;
% 
%             end
%             S = cycle_data(:, 2, turbine_index) ./ (1 - cycle_data(:, 3, turbine_index) ./ theta_b);
%             % 计算L_p
...
%             % 计算L_p
%             L_p = (sum(S.^m .* cycle_data(:, 1, turbine_index) ./ N_total))^(1/m);
%     %         disp([num2str(current_index),'风机 ', num2str(turbine_index), ' 的 L_p = ', num2str(L_p)]);
%             L_N(current_index, turbine_index) = L_p;
%             % 计算N
%             S = cycle_data(count, 2, turbine_index) ./ (1 - cycle_data(count, 3, turbine_index) ./ theta_b);
%             N = C ./ S.^m;
%             % 计算Df_p
%             Df_p = sum(cycle_data(count, 1, turbine_index) ./ N);
%             D_F(current_index, turbine_index) = Df_p+D_F(current_index-1, turbine_index);
%     %         disp([num2str(current_index),'风机 ', num2str(turbine_index), ' 的 Df_p = ', num2str(Df_p)]);
%             last_count(turbine_index) = count;
%         end
%     end
end
