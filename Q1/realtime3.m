...
% 定义寻找局部极大值和极小值的函数
function findExtrema(current_index, turbine_index,raw_data)
    ...
    persistent half_cycle_count full_cycle_count cycle_data cycle_count L_N D_F;
    persistent theta_b mm C N_total;
    ...
    maxcycle_count = half_cycle_count(turbine_index)+half_cycle_count(turbine_index);
    if maxcycle_count>cycle_count(turbine_index)
        while(maxcycle_count>cycle_count(turbine_index))
        cycle_count(turbine_index)=cycle_count(turbine_index)+1;
        % 计算S
        S = cycle_data(cycle_count(turbine_index), 2, turbine_index) ./ (1 - cycle_data(cycle_count(turbine_index), 3, turbine_index) ./ theta_b);            
        % 计算L_p
        L_p = (sum(S.^mm .* cycle_data(cycle_count(turbine_index), 1, turbine_index) ./ N_total))^(1/mm);
    %             disp(['风机 ', num2str(turbine_index), ' 的 L_p = ', num2str(L_p)]);
        L_N(current_index, turbine_index) = L_N(current_index-1, turbine_index)+L_p;
        % 计算N
        N = C ./ S.^mm;
        % 计算Df_p
        Df_p = sum(cycle_data(cycle_count(turbine_index), 1, turbine_index) ./ N);
        D_F(current_index, turbine_index) = D_F(current_index-1, turbine_index)+Df_p;
        end
    else
...
% 定义雨流计数函数并在每次循环后进行计算
function rainflowCounting(current_index,turbine_index,minima_stack,maxima_stack,Ptrmin,Ptrmax)
    persistent half_cycle_count full_cycle_count cycle_data L_N D_F;
    persistent theta_b mm C N_total;
    ...
    while Ptrmin(turbine_index) >= 1 && Ptrmax(turbine_index) >= 1
        ...
        if Ptrmax(turbine_index) == 1
            ...
            cycle_count = half_cycle_count(turbine_index)+half_cycle_count(turbine_index);
            % 计算S
            S = cycle_data(cycle_count, 2, turbine_index) ./ (1 - cycle_data(cycle_count, 3, turbine_index) ./ theta_b);            
            % 计算L_p
            L_p = (sum(S.^mm .* cycle_data(cycle_count, 1, turbine_index) ./ N_total))^(1/mm);
%             disp(['风机 ', num2str(turbine_index), ' 的 L_p = ', num2str(L_p)]);
            L_N(current_index, turbine_index) = L_N(current_index-1, turbine_index)+L_p;
            % 计算N
            N = C ./ S.^mm;
            % 计算Df_p
            Df_p = sum(cycle_data(cycle_count, 1, turbine_index) ./ N);
            D_F(current_index, turbine_index) = D_F(current_index-1, turbine_index)+Df_p;
...
%             disp(['风机 ', num2str(turbine_index), ' 的 L_p = ', num2str(L_p)]);
        ...
        % 检查是否需要递归处理
        if Ptrmin(turbine_index) >= 1
            rainflowCounting(current_index,turbine_index,minima_stack,maxima_stack,Ptrmin,Ptrmax)
        end
    end
